...
# Advanced Bacterial Dynamics Model — Persistence, Proportions, Treatment, White PNGs
# -----------------------------------------------------------------------------------
...
CONFIG <- list(
  ...
  DEBUG_MODE = TRUE,
  SIMULATION_DURATION = 365.25 * 20,  # 20 years
  RANDOM_SEED = 42
)
...
# Safe CSV writer (handles file locks) --------------------------------------------
...
# Parameters (with fitness cost + treatment) --------------------------------------
prepare_model_parameters <- function(demographic_data) {
  ...
  list(
    ...
    resistance_cost      = 0.15,   # penalize resistant
    treatment_rate       = 0.001,  # per-day treatment flow S->Sr, R->Rs
    treatment_efficacy   = 0.5,    # treated hosts transmit at 50%
    
    base_mortality       = 0.001   # (placeholder)
  )
}

# ODEs (mass-conserving + treatment + fitness cost) --------------------------------
create_bacterial_odes <- function(n_age, contact_matrix, births_vector) {
  function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      # Effective params for resistant
      beta_R_eff  <- (1 - resistance_cost) * beta_resistant
      clear_R_eff <- (1 + resistance_cost) * clearance_resistant
      
      # Treated transmit less
      infectious_S <- S + (1 - treatment_efficacy) * Sr
      infectious_R <- R + (1 - treatment_efficacy) * Rs
      
      total_population <- X + S + R + Sr + Rs
      denom <- pmax(total_population, 1e-10)
      
      contact_weighted_S <- contact_matrix %*% (infectious_S / denom)
      contact_weighted_R <- contact_matrix %*% (infectious_R / denom)
      
      lambda_S <- as.numeric(beta_sensitive * contact_weighted_S)
      lambda_R <- as.numeric(beta_R_eff      * contact_weighted_R)
      
      # Mass-conserving flows: all clearances return to X
      dX  <- births_vector -
        lambda_S * X - lambda_R * X +
        clearance_sensitive * S  + clear_R_eff * R +
        clearance_sensitive * Sr + clear_R_eff * Rs
      
      dS  <-  lambda_S * X
      - clearance_sensitive * S
      - co_colonization_rate * lambda_R * S
      - treatment_rate * S
      
      dR  <-  lambda_R * X
      - clear_R_eff * R
      - co_colonization_rate * lambda_S * R
      - treatment_rate * R
      
      dSr <-  co_colonization_rate * lambda_R * S
      - clearance_sensitive * Sr
      + treatment_rate * S
      
      dRs <-  co_colonization_rate * lambda_S * R
      - clear_R_eff * Rs
      + treatment_rate * R
...
plot_total_counts <- function(results_long) {
  results_long %>% dplyr::group_by(time, group) %>%
    ...
    ggplot2::guides(color = ggplot2::guide_legend(title = "Compartment")) +
    theme_clean()
}
...
plot_strain_prevalence <- function(prev_df) {
  ggplot2::ggplot(prev_df, ggplot2::aes(time, prop_strain, fill = strain)) +
    ...
    ggplot2::guides(fill = ggplot2::guide_legend(title = "Strain")) +
    theme_clean()
}
...
plot_age_profile_at_times <- function(results_long,
  ...
  ggplot2::ggplot(df_plot, ggplot2::aes(age, prop, color = strain, fill = strain)) +
    ...
    ggplot2::guides(color = ggplot2::guide_legend(title = "Strain"),
                    fill  = ggplot2::guide_legend(title = "Strain")) +
    theme_clean()
}

# Save wrapper (white PNG background) ----------------------------------------------
save_plot <- function(plot, filename, width, height, dpi = 150) {
  ...
  ggplot2::ggsave(
    path, plot = plot, width = width, height = height, dpi = dpi, bg = "white"
  )
