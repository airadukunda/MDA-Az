...
bacteria.odes <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    #...........................................................................
    #MDA intervention into the ODE
    mda_targeted_ages <- 0:5         #       Targeted ages 
    azt <- rep(0, n_age)             #       Initialize azt vector
    azt[mda_targeted_ages] <- 0.8    # or 1  MDA coverage 
    #a_t
    b <- ifelse(mda_active(t, mda_start_times, mda_duration),a + (a.use.eff*a.use), (a.use.eff*a.use))
    a_t <- b * azt
    #a.C_t
    bc <- ifelse(mda_active(t, mda_start_times, mda_duration),a.C + (a.use.eff*a.use), (a.use.eff*a.use))
    a.C_t <- bc * azt
    ...
    #
    dR <- lamda.R * X - u.R * R - k * lamda.S * R + a.C_t * (Sr + Rs) +  ageing %*% R - mort_eff * R -amrd_rate * R         #
    #
    dSr <- k * lamda.R * S - Sr * u.C - a.C_t * Sr  +  ageing %*% Sr - mort_eff * Sr                                        #
    #
    dRs <- k * lamda.S * R - Rs * u.C - a.C_t * Rs  + ageing %*% Rs - mort_eff * Rs                                         #
    #
...
parms.orig <- list(
  ...
  a.use = 0.06,            # Antibiotic use (~0.01–0.05:routine use,0.05 – 0.20: High use communitie,0.20 – 0.80 (short period) :MDA  )
  ...
  amrd_rate = 27.3/(100000*365) #AMR related mortalty per person per day 
  )
...
state.orig<-c(initX,initS,initR,initSr,initRs,initD,initCumR,initAMRD)
#States
...
mda_start_times <- (0:10) * (365.25/1) #annual
out_50_a_Tanzania <- bacteria.solve(tvec_50_a, state,parms)
mda_start_times <- (0:10) * (365.25/2) # bi annual
...
# Column names 
compartment_names <- c("X", "S", "R", "Sr", "Rs","D","CumR","AMRD")
col_names <- c("time")
...
#A. Resistance  prevalence  # We considered on R as in routine surveillance, resistant cases are reported 
#---
R_1_a <- round(rowSums(out_1_a_Tanzania[, Rindex + 1]) * 100 / rowSums(out_1_a_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_1_b <- round(rowSums(out_1_b_Tanzania[, Rindex + 1]) * 100 / rowSums(out_1_b_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_1_c <- round(rowSums(out_1_c_Tanzania[, Rindex + 1]) * 100 / rowSums(out_1_c_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_5_a <- round(rowSums(out_5_a_Tanzania[, Rindex + 1]) * 100 / rowSums(out_5_a_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_5_b <- round(rowSums(out_5_b_Tanzania[, Rindex + 1]) * 100 / rowSums(out_5_b_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_5_c <- round(rowSums(out_5_c_Tanzania[, Rindex + 1]) * 100 / rowSums(out_5_c_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_10_a <- round(rowSums(out_10_a_Tanzania[, Rindex + 1]) * 100 / rowSums(out_10_a_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_10_b <- round(rowSums(out_10_b_Tanzania[, Rindex + 1]) * 100 / rowSums(out_10_b_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_10_c <- round(rowSums(out_10_c_Tanzania[, Rindex + 1]) * 100 / rowSums(out_10_c_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_50_a <- round(rowSums(out_50_a_Tanzania[, Rindex + 1]) * 100 / rowSums(out_50_a_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_50_b <- round(rowSums(out_50_b_Tanzania[, Rindex + 1]) * 100 / rowSums(out_50_b_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
R_50_c <- round(rowSums(out_50_c_Tanzania[, Rindex + 1]) * 100 / rowSums(out_50_c_Tanzania[, c(Xindex, Sindex, Rsindex, Srindex, Rindex) + 1]), 1)
...
plot_1<-ggplot(df_all_long, aes(x = Days, y = Resistance, color = Scenario)) +
  geom_line() +
  geom_hline(
    yintercept = 15,
    color      = "red",
...
df_all_long_1<-df_all_long %>%
  filter(Scenario != "Baseline")
plot_2<-ggplot(df_all_long_1, aes(x =Scenario, y = Resistance, color = Scenario)) +
  geom_boxplot(color="black",fill="skyblue") +
  geom_hline(yintercept = 15,               # horizontal line at 15%
    color = "red", 
    linetype = "dashed", 
    size = 1) +
...
df_c_all_long_1<-df_c_all_long %>%
  filter(Scenario != "Baseline")
plot_4<-ggplot(df_c_all_long_1, aes(x =Scenario, y = Resistance, color = Scenario)) +
  geom_boxplot(color="black",fill="skyblue") +
  geom_hline(yintercept = 15,               # horizontal line at 15%
    color = "red", 
    linetype = "dashed", 
...
ggplot(df_with_resisitance_mortality, aes(x = time, y = prevalence)) +
  ...
  labs(
    ...
    subtitle="Antibiotic use in Tanzania : 6%",
    x = "Time",
    y = "Resistance (%)"
  ) +
...
cases<-ggplot(totalresistance, aes(x = Scenario, y = R_final, fill = Scenario)) +
  geom_bar(stat = "identity", color = "black") +
  scale_y_continuous(labels = scales::label_number(scale = 1e-9, accuracy = 1, suffix = " M ")) +  
  scale_fill_viridis_d(option = "viridis") +
  labs(
...
Deaths_averted_plot<-ggplot(Deaths_averted , aes(x = Scenario, y = Deaths_averted_millions, fill = Scenario)) +
  geom_bar(stat = "identity", color = "black") +
  scale_y_continuous(labels = scales::label_number(scale = 1e-6, accuracy = 1, suffix = " M ")) +  
  scale_fill_viridis_d(option = "viridis") +
  labs(
...
ggsave("Graph 1.annual_MDA.png", plot = graph_1_1, width = 8, height = 6)
  ...
  adjust_size(width = 80, height = 105) |>
  add_mean_line() |>
  #add_mean_dot() |>
  add_sem_ribbon()|>
  view_plot(title = "A.Annual MDA")
...
g_0_1<-ggplot(dataset_1_long, aes(x = time, y = Proportion, color = scenario)) +
  ...
  geom_line(linewidth = 1) +
  scale_x_continuous(
    breaks = seq(min(dataset_1_long$time), max(dataset_1_long$time), by = 3650)
  ) +
  scale_y_continuous(
    limits = c(0, 100),
    breaks = seq(0, 100, by = 10))+
...
g8 <- grid.arrange(
  g_0_1,g4, g_0_5,g5, g_0_10, g6, nrow = 3,
