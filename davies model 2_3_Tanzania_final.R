...
bacteria.odes <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    mort_eff <- mort  # start from baseline (to be verified)
    ...
    dX <- births + (u.S + a_t) * S + u.R * R + u.C * (Sr + Rs) - ( lamda.S + lamda.R ) * X +  ageing %*% X - mort_eff * X     # + MDA * S # MDA * (1-kappa) * S  # Successfully treated return to X 
