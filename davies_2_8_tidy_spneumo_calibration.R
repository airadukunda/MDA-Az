...
baseline_parameters <- list(
  # Transmission: calibrate beta.S; do not trust the starting value.
  beta.S_start = 0.03,

  # Resistance biology.
  c = 0.05, # fitness cost of macrolide resistance; sensitivity 0.00-0.15
  ...
  macrolide_use_ddd_per_1000_per_day = 2,
...
make_parameters <- function(inputs, indices, ageing) {
  ...
  tau <- (baseline_parameters$macrolide_use_ddd_per_1000_per_day / 1000) /
...
make_spneumo_age_parameters <- function(age_groups) {
  ...
  list(
    ...
    u.R_monthly_age = u.S_monthly_age * 1.05,
...
plot_macrolide_resistance_time_series <- function(time_series) {
  time_series |>
    ...
    ggplot2::labs(
      ...
      x = "Time (years)",
      y = "Macrolide-resistant among pneumococcal-carriers (%)",
      colour = "Scenario"
    ) +
...
message("Base scenario outputs written to: ", normalizePath(config$output_dir, mustWork = FALSE))


# -----------------------------------------------------------------------------
# 11. Pneumococcal calibration outputs and age-band diagnostics
# -----------------------------------------------------------------------------
# This section is intentionally placed at the bottom so it can use the model
# objects created above: inputs, indices, parameters, equilibrium_output,
# all_results, and config.
...
# Initial calibration targets. Replace these with local post-PCV carriage data
# when available. Values are proportions, not percentages.
carriage_targets <- tibble::tibble(
  ...
  target_resistant_among_carried = c(0.12, 0.12, 0.10, 0.08, 0.08),
...
plot_resistant_fraction_among_carriers_over_time <- function(time_series_by_age,
  ...
  time_series_by_age |>
    ...
    ggplot2::facet_wrap(~ age_band, scales = "free_y") +
    ggplot2::labs(
      title = paste0("Macrolide-resistant fraction among carriers over ", horizon, " years"),
      x = "Time since scenario start (years)",
      y = "Macrolide-resistant among carriers (%)",
      colour = "Scenario"
    ) +
