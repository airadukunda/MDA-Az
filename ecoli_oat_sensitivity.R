# =============================================================================
# One-at-a-time sensitivity analysis for E. coli MDA model
# =============================================================================
# -----------------------------------------------------------------------------
# 0. Load calibrated model
# -----------------------------------------------------------------------------

source("davies_2_8_tidy_ecoli.R")

# Put sensitivity outputs in a subfolder.
base_output_dir <- config$output_dir
config$output_dir <- file.path(base_output_dir, "sensitivity_oat_ecoli")
...
make_oat_specs_ecoli <- function(bp) {
  max_mda_effect_multiplier <- if ("mda_p_clear_S" %in% names(bp)) {
    min(1 / bp$mda_p_clear_S, 1 / bp$mda_p_select_C)
  } else {
    1.5
  }
  ...
  tibble::tribble(
    ...
    "Maximum plausible MDA treatment effect given baseline parameterisation",
    ...
    "amrd_rate_low", "amrd_rate_multiplier", "low", 0.50,
    "Half baseline AMR-attributable mortality rate",
    "amrd_rate_high", "amrd_rate_multiplier", "high", 2.00,
    "Double baseline AMR-attributable mortality rate",
    "theta_low", "theta", "low", 0.10,
    "Lower all-cause mortality effect of MDA",
    "theta_high", "theta", "high", 0.18,
    "Higher all-cause mortality effect of MDA"
  )
}

oat_specs <- make_oat_specs_ecoli(baseline_parameters_reference)

# -----------------------------------------------------------------------------
# 2. Apply one sensitivity change to baseline_parameters
# -----------------------------------------------------------------------------

apply_ecoli_sensitivity_change <- function(bp, parameter, value) {
  ...
  if (parameter == "amrd_rate_multiplier") {
    bp$amrd_rate <- baseline_parameters_reference$amrd_rate * value
    return(bp)
  }
...
summarise_age_group_endpoint <- function(time_series_by_age,
  ...
  time_series_by_age |>
    ...
    dplyr::summarise(
      ...
      colonisation_prevalence = dplyr::if_else(
        total_population > 0,
...
add_no_mda_comparison_age_group <- function(group_endpoints) {
  no_mda <- group_endpoints |>
    ...
    dplyr::select(
      ...
      no_mda_colonisation_prevalence = colonisation_prevalence,
  ...
  group_endpoints |>
    ...
    dplyr::mutate(
      ...
      colonisation_difference_percentage_points =
        colonisation_prevalence - no_mda_colonisation_prevalence,
...
run_ecoli_oat_case <- function(case_row) {
  ...
  bp_case <- apply_ecoli_sensitivity_change(
...
oat_results <- purrr::map(
  ...
  function(i) {
    tryCatch(
      run_ecoli_oat_case(oat_specs[i, ]),
      error = function(e) {
        warning(
        ...
        list(
          case_summary = tibble::tibble(
            ...
            colonisation_prevalence = NA_real_,
...
plot_oat_tornado_absolute_faceted <- function(tornado_data,
                                              title = "E. coli sensitivity: absolute resistance prevalence",
                                              x_label = "10-year resistant among colonised (%)") {
  ggplot2::ggplot(
    tornado_data,
    ggplot2::aes(y = reorder(parameter, abs(range)))
  ) +
...
plot_oat_endpoint_points <- function(sensitivity_summary,
  ...
  sensitivity_summary |>
    ...
    ggplot2::labs(
      title = paste0(
        "E. coli OAT sensitivity: ",
        scenario_name,
        ", ",
        horizon,
        " years"
      ),
      x = "Increase in resistant among colonised vs no MDA (percentage points)",
      y = NULL
    ) +
    ggplot2::theme_classic(base_size = 12)
}
...
make_parameter_range_table <- function(oat_specs,
  ...
  baseline_row <- tibble::tibble(
    ...
    baseline_value = c(
      ...
      1,
      1,
      baseline_parameters_reference$theta
    ),
    baseline_parameter_value = c(
      ...
      mda_effect_baseline,
      baseline_parameters_reference$amrd_rate,
      baseline_parameters_reference$theta
    )
  )
  ...
  baseline_row |>
    ...
    dplyr::mutate(
      parameter_description = dplyr::case_when(
        ...
        parameter == "amrd_rate_multiplier" ~ "AMR-attributable mortality multiplier",
        parameter == "theta" ~ "All-cause mortality reduction during active MDA",
        TRUE ~ parameter
      )
    ) |>
    dplyr::select(
...
readr::write_csv(oat_specs, output_path("ecoli_oat_sensitivity_specs.csv"))
readr::write_csv(sensitivity_summary, output_path("ecoli_oat_sensitivity_summary.csv"))
readr::write_csv(sensitivity_time_series_all, output_path("ecoli_oat_time_series_all_ages.csv"))
readr::write_csv(sensitivity_time_series_by_age, output_path("ecoli_oat_time_series_by_age.csv"))
...
readr::write_csv(
  ...
  output_path("ecoli_oat_tornado_biannual_10y_all_ages.csv")
)

readr::write_csv(
  tornado_annual_10y_all,
  output_path("ecoli_oat_tornado_annual_10y_all_ages.csv")
)

tornado_biannual_plot <- plot_oat_tornado(
  tornado_biannual_10y_all,
  title = "E. coli sensitivity: biannual MDA at 10 years"
)

tornado_annual_plot <- plot_oat_tornado(
  tornado_annual_10y_all,
  title = "E. coli sensitivity: annual MDA at 10 years"
)
...
save_plot(
  ...
  "ecoli_oat_tornado_biannual_10y_all_ages.png",
  width = 8,
  height = 5
)
...
save_plot(
  tornado_annual_plot,
  "ecoli_oat_tornado_annual_10y_all_ages.png",
  width = 8,
  height = 5
)
...
save_plot(
  ...
  "ecoli_oat_endpoint_points_biannual_10y_all_ages.png",
  width = 8,
  height = 6
)
...
readr::write_csv(
  absolute_tornado_no_mda,
  output_path("ecoli_oat_tornado_absolute_no_mda_10y_all_ages.csv")
)
readr::write_csv(
  absolute_tornado_annual,
  output_path("ecoli_oat_tornado_absolute_annual_10y_all_ages.csv")
)
readr::write_csv(
  absolute_tornado_biannual,
  output_path("ecoli_oat_tornado_absolute_biannual_10y_all_ages.csv")
)
readr::write_csv(
  absolute_tornado_all,
  output_path("ecoli_oat_tornado_absolute_10y_all_ages_by_scenario.csv")
)

absolute_tornado_no_mda_plot <- plot_oat_tornado(
  absolute_tornado_no_mda,
  title = "E. coli sensitivity: No MDA at 10 years",
  x_label = "Resistant among colonised at 10 years (%)"
)
...
absolute_tornado_faceted_plot <- plot_oat_tornado_absolute_faceted(
  absolute_tornado_all,
  title = "E. coli sensitivity: 10-year resistance prevalence by scenario",
  x_label = "Resistant among colonised at 10 years (%)"
)

save_plot(
  absolute_tornado_no_mda_plot,
  "ecoli_oat_tornado_absolute_no_mda_10y_all_ages.png",
  width = 8,
  height = 5
)
...
save_plot(
  ...
  "ecoli_oat_tornado_absolute_annual_10y_all_ages.png",
  width = 8,
  height = 5
)
...
save_plot(
  ...
  "ecoli_oat_tornado_absolute_biannual_10y_all_ages.png",
  width = 8,
  height = 5
)
...
save_plot(
  ...
  "ecoli_oat_tornado_absolute_10y_all_ages_by_scenario.png",
  width = 12,
  height = 5.5
)

# Net-benefit tornado plots for E. coli mortality module.
make_tornado_data_net_deaths <- function(sensitivity_summary,
                                         scenario_name = "Biannual MDA",
                                         horizon = 10,
                                         outcome_group_name = "all_ages") {
  make_tornado_data_absolute(
    ...
    outcome = "net_deaths_averted"
  )
}

tornado_net_deaths_biannual <- make_tornado_data_net_deaths(
  sensitivity_summary,
  scenario_name = "Biannual MDA",
  horizon = 10,
  outcome_group_name = "all_ages"
...
readr::write_csv(
  tornado_net_deaths_biannual,
  output_path("ecoli_oat_tornado_net_deaths_biannual_10y_all_ages.csv")
)

net_deaths_tornado_plot <- plot_oat_tornado(
  tornado_net_deaths_biannual,
  title = "E. coli sensitivity: net deaths averted, biannual MDA at 10 years",
  x_label = "Net deaths averted vs no MDA"
)

save_plot(
  net_deaths_tornado_plot,
  "ecoli_oat_tornado_net_deaths_biannual_10y_all_ages.png",
  width = 8,
  height = 5
)
...
readr::write_csv(
  ...
  output_path("ecoli_oat_parameter_ranges.csv")
)
...
writeLines(
  ...
  con = output_path("ecoli_oat_parameter_ranges.md")
)
...
saveRDS(
  ...
  file = output_path("ecoli_oat_sensitivity_outputs.rds")
)

message("E. coli OAT sensitivity analysis complete.")
